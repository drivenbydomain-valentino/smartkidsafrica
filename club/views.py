...
def schoolregister(request):
    if request.method == "POST":
        ...
        # 🔥 IMPORTANT FIX: check school_name instead of email
        if User.objects.filter(username=school_name).exists():
            return render(request, "club/schoolregister.html", {
                "error": "School name already exists"
            })

        # ✅ create user using school_name
        user = User.objects.create_user(
            username=school_name,   # ✅ FIXED
            email=email,
            password=password
        )
...
def school_login(request):
    if request.method == "POST":
        ...
        user = authenticate(request, username=school_name, password=password)
